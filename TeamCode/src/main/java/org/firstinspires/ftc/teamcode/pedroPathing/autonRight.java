...
@Autonomous(name = "Auton bottom right", group = "Autonomous")
@Configurable // Panels
public class autonRight extends OpMode {
    ...
    public void init() {
        ...
        follower.setStartingPose(new Pose(85, 10, Math.toRadians(225)));
    ...
    public static class Paths {
        ...
        public Paths(Follower follower) {
            Path1 = follower
                    .pathBuilder()
                    .addPath(
                            new BezierLine(new Pose(85.000, 10.000), new Pose(109.347, 107.910))
                    )
                    .setConstantHeadingInterpolation(Math.toRadians(225)) // Linear doesn't seem to work for some reason
                    .build();
        }
    }
    ...
    public int autonomousPathUpdate() {
        switch (pathingState) {
            case "":
                follower.followPath(paths.Path1); // REMOVE IF BEFORE COMP
